 ...
 * @LastEditors: anan
 * @LastEditTime: 2020-09-05 14:48:38
 -->
<template>
...
import { getProportionOfSales } from '@/api/echarts' // getCoupon,
...
export default {
  ...
  methods: {
    initCharts() {
      getProportionOfSales().then(response => {
        debugger
        const proportionOfSales = response.data
        // 渲染图例组件
        this.renderLegend(proportionOfSales)
        // 渲染维度
        this.renderXAxis(proportionOfSales)
        // 渲染列表
        this.renderSeries(proportionOfSales)
        // 获取图表div
    ...
    renderLegend(data) {
      data.forEach(proportion => {
        if (!this.legendData.includes(proportion.months)) {
          this.legendData.push(proportion.months)
        }
      })
    },
    // 渲染维度
    renderXAxis(data) {
      data.forEach(proportion => {
        if (proportion.months === this.legendData[0]) {
          this.xAxisData.push(proportion.name)
        }
      })
    },
    // 渲染列表
    renderSeries(data) {
      ...
      for (let i = 0; i < this.legendData.length; i++) {
        ...
        const arr = []
        data.forEach(proportion => {
          if (proportion.months === series) {
            arr.push(proportion.bl)
          }
        })
