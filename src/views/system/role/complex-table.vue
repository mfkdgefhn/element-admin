<template>
  <div class="app-container">
    ...
    >
      ...
      <!-- 权限字符 -->
      <el-table-column :label="$t('roletable.perChar')" align="center" width="300px">
        <template slot-scope="scope">
          <span>{{ scope.row.perChar }}</span>
        </template>
      </el-table-column>

      <!-- 显示顺序 -->
      <el-table-column :label="$t('roletable.displayOrder')" align="center">
        <template slot-scope="scope">
      ...
      <!-- 角色状态 -->
      <el-table-column :label="$t('roletable.roleStatus')" align="center">
        <template slot-scope="{row}">
          <el-tag :type="row.roleStatus | statusFilter">{{ row.roleStatus }}</el-tag>
        </template>
      </el-table-column>

      <!-- 时间-->
      <!-- <el-table-column :label="$t('roletable.createDate')" class-name="status-col">
        <template slot-scope="{row}">
      ...
      <!-- 创建时间 -->
      <el-table-column :label="$t('roletable.createDate')" align="center">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="{row}">
          ...
          >{{ $t('roletable.dataPermissions') }}</el-button>
          <!-- 分配用户 -->
          <el-button
            v-if="row.status!='allocateUsers'"
            size="mini"
            @click="handleModifyStatus(row,'allocateUsers')"
          >{{ $t('roletable.allocateUsers') }}</el-button>
          <!-- 删除 -->
          <el-button
...
<script>
import { /* fetchList, */ fetchRoleList, fetchPv, createArticle, updateArticle } from '@/api/article'
